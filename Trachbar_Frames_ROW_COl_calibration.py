...
while True:
    ...
    if retL and retR == True:
        ...
        Checker_Board_functions.draw_first_point(img_l_fast,cornersL_fast, color=(255, 255, 0), radius=5)
        Checker_Board_functions.draw_last_point(img_l_fast, cornersL_fast, color=(0, 255, 0), radius=5)
        Checker_Board_functions.draw_first_point(img_r_fast,cornersR_fast, color=(255, 255, 0), radius=5)
        Checker_Board_functions.draw_last_point(img_r_fast, cornersR_fast, color=(0, 255, 0), radius=5)
...
# retL, cameraMatrixL, distL, rvecsL, tvecsL = cv.calibrateCamera(objpoints, imgpointsL, (width, height), None, None)
# newCameraMatrixL, roi_L = cv.getOptimalNewCameraMatrix(cameraMatrixL, distL, (width,height), 1, (width,height))
#
# retR, cameraMatrixR, distR, rvecsR, tvecsR = cv.calibrateCamera(objpoints, imgpointsR, (width, height), None, None)
# newCameraMatrixR, roi_R = cv.getOptimalNewCameraMatrix(cameraMatrixR, distR, (width, height), 1, (width, height))
#
...
# retStereo, StereoCameraMatrixL, distL_stereo, StereoCameraMatrixR, distR_stereo, rot, trans, essentialMatrix, fundamentalMatrix = cv.stereoCalibrate(
#     objpoints, imgpointsL, imgpointsR, newCameraMatrixL, distL, newCameraMatrixR, distR, (width, height), criteria_stereo, flags)
# print('rot', rot)
#
...
#                                                                                            (width, height), rot, trans, rectifyScale, (0, 0))
#
# stereoMapL = cv.initUndistortRectifyMap(newCameraMatrixL, distL_stereo, rectL_stereo, projMatrixL, (width, height), cv.CV_16SC2)
# stereoMapR = cv.initUndistortRectifyMap(newCameraMatrixR, distR_stereo, rectR_stereo, projMatrixR, (width, height), cv.CV_16SC2)
#
# print("Saving parameters!")
# cv_file = cv.FileStorage('stereoMap3.xml', cv.FILE_STORAGE_WRITE)
#
